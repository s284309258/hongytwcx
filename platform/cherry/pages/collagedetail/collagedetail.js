...
let goodsList = [
  {actEndTime: '2020/05/28 10:00:43'},]
Page({
  data: {
  ...
  buyGoods: function () {
    var that = this;
      ...
      // 直接购买商品
      util.request(api.BuyAdd, { goodsId: this.data.goods.id, number: this.data.number, productId: checkedProduct[0].id }, "POST",'application/json').then(function (res) {
          let _res = res;
          if (_res.errno == 0) {
            that.setData({
            ...
            wx.navigateTo({
              url: '/pages/shopping/checkout/checkout?isBuy=2',
            })
          } else {
            wx.showToast({
              image: '/static/images/icon_error.png',
