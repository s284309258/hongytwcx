$(function () {
    ...
    $("#jqGrid").Grid({
        ...
        colModel: [
            ...
            {
                label: '订单类型', name: 'orderType', index: 'order_type', width: 90, formatter: function (value) {
                    if (value == '1') {
                        return '普通订单';
                    } else if (value == '2') {
                        return '团购订单';
            ...
            {
                label: '订单状态', name: 'orderStatus', index: 'order_status', width: 100, formatter: function (value) {
                    if (value == '0') {
                        return '待付款';
                    } else if (value == '101') {
                        return '订单已取消';
            ...
            {
                label: '下单时间', name: 'addTime', index: 'add_time', width: 90,
                formatter: function (value) {
            ...
            },
            {
                label: '操作', width: 160, align: 'center', sortable: false, formatter: function (value, col, row) {
                    return '<button class="btn btn-outline btn-info" onclick="vm.lookDetail(' + row.id + ')"><i class="fa fa-info-circle"></i>&nbsp;详情</button>' +
                        ...
                        '<button class="btn btn-outline btn-primary" style="margin-top: 10px;" onclick="vm.refund(' + row.id + ')"><i class="fa fa-print"></i>&nbsp;退款</button>';
                }
            }
        ]
    });
...
let vm = new Vue({
    ...
    methods: {
        ...
        refund: function(rowId){
            confirm('确定退款？', function (r){
                if(r){
                    Ajax.request({
                        type: "POST",
                        url: "../order/refund/" + rowId,
                        async: true,
                        successCallback: function (r) {
                            vm.reload();
                        }
