...
import com.platform.service.ApiCommonService;
...
public class ApiPayController extends ApiBaseAction {
    ...
    public Object orderQuery(@LoginUser UserVo loginUser, Integer orderId) {
        ...
        if ("SUCCESS".equals(trade_state)) {
            ...
            if(!"2".equals(orderDetail.getOrder_type())){
    ...
    public void notify(HttpServletRequest request, HttpServletResponse response) {
        try {
            ...
            if (result_code.equalsIgnoreCase("FAIL")) {
                ...
                UserVo userVo = userService.queryObject(orderInfo.getUser_id());
                UserVo ref_UserVo = userService.queryObject(userVo.getReference_id());
                if(out_trade_no.startsWith("YTP")){
                    if(orderInfo.getPay_status()==1){
                        orderInfo.setPay_status(2);
                    }
                    if(orderInfo.getOrder_status()==0){
                        orderInfo.setOrder_status(107);
                    }
                    orderInfo.setShipping_status(0);
                    orderInfo.setPay_time(new Date());
                    orderService.update(orderInfo);
                    //更新任务量和升级会员插入消费明细
                    apiCommonService.UpdateUserTaskNumAndUpgrade(userVo,null,orderInfo).
                            PinDanLogicHandle(orderInfo).
                            InsertConsumerRecord(userVo,orderInfo);
                }else{
                    if(orderInfo.getPay_status()==1){
                        orderInfo.setPay_status(2);
                    }
                    ...
                    //更新支付状态和发货状态
                    orderService.update(orderInfo);
                    //更新任务量和升级会员更新直属收益插入消费明细
                    apiCommonService.UpdateUserTaskNumAndUpgrade(userVo,null,orderInfo).
                            UpdateSeniorUserBenefit(ref_UserVo,orderInfo).
                            InsertConsumerRecord(userVo,orderInfo);
                }
            }
        } catch (Exception e) {
...
//                orderInfo.getActual_price().doubleValue(), orderInfo.getActual_price().doubleValue());
        WechatRefundApiResult result = WechatUtil.wxRefund(orderInfo.getOrder_sn(),
                Double.parseDouble(orderInfo.getOrder_price().setScale(2,RoundingMode.DOWN).toString()),
                Double.parseDouble(orderInfo.getOrder_price().setScale(2,RoundingMode.DOWN).toString()));
        if (result.getResult_code().equals("SUCCESS")) {
            ...
            }else if(orderInfo.getOrder_status()==109){
                orderInfo.setOrder_status(110);
            }
            orderService.update(orderInfo);
