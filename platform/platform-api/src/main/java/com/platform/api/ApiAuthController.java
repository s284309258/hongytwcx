...
public class ApiAuthController extends ApiBaseAction {
    ...
    public Object loginByWeixin() {
        ...
        logger.info("login_by_weixin 请求参数{}"+jsonParam);
        String code = "";
        ...
        Long spid=jsonParam.get("spid")!=null?Long.parseLong(jsonParam.get("spid").toString()):0L;
        UserVo ref_UserVo = userService.queryObject(spid);
        //会员授权登陆生成会员上下级关系,父级链,会员初始级别
        apiCommonService.UpdateUserTaskNumAndUpgrade(ref_UserVo,userVo,null).
                GenerateSeniorRelation(userVo,ref_UserVo).
                UpdateLoginUserInfo(userVo,userInfo,sessionData);
