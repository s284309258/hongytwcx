...
public class ApiCartController extends ApiBaseAction {
    ...
    public Object add(@LoginUser UserVo loginUser) {
        ...
        if (null == cartInfo) {
            ...
            cartInfo.setGoods_sn(String.valueOf(productInfo.getGoods_sn()));
    ...
    @PostMapping("update")
    public Object update(@LoginUser UserVo loginUser,@RequestParam Map paramsMap) {
//        JSONObject jsonParam = getJsonRequest();
        Integer goodsId = Integer.valueOf(paramsMap.get("goodsId").toString());
        Integer productId = Integer.valueOf(paramsMap.get("productId").toString());
        Integer number = Integer.valueOf(paramsMap.get("number").toString());
        Integer id = Integer.valueOf(paramsMap.get("id").toString());
        //取得规格的信息,判断规格库存
        ProductVo productInfo = productService.queryObject(productId);
        if (null == productInfo || productInfo.getGoods_number() < number) {
            return this.toResponsObject(400, "库存不足", "");
        ...
        if (null == newcartInfo) {
            ...
            cartInfo.setProduct_id(productId);
            cartInfo.setGoods_sn(String.valueOf(productInfo.getGoods_id()));
            cartInfo.setNumber(number);
            cartInfo.setRetail_price(productInfo.getRetail_price());
            cartInfo.setMarket_price(productInfo.getRetail_price());
            if (null != goodsSepcifitionValue) {
    ...
    public Object checkout(@LoginUser UserVo loginUser, Integer couponId, @RequestParam(defaultValue = "cart") String type) {
        ...
        if (type.equals("cart")) {
            ...
            for (CartVo cartEntity : (List<CartVo>) cartData.get("cartList")) {
                //取得规格的信息,判断规格库存
                ProductVo productInfo3 = productService.queryObject(cartEntity.getProduct_id());
                if (null == productInfo3 || productInfo3.getGoods_number() < cartEntity.getNumber()) {
                    return this.toResponsObject(400, cartEntity.getGoods_name()+"库存不足", "");
                }
                if("0".equals(productInfo3.getIs_on_sale())){
                    return this.toResponsObject(400, cartEntity.getGoods_name()+"已下架", "");
                }
            ...
            //取得规格的信息,判断规格库存
            ProductVo productInfo3 = productService.queryObject(goodsVO.getProductId());
            if (null == productInfo3 || productInfo3.getGoods_number() < goodsVO.getNumber()) {
                return this.toResponsObject(400, productInfo3.getGoods_name()+"库存不足", "");
            }
            if("0".equals(productInfo3.getIs_on_sale())){
                return this.toResponsObject(400, productInfo3.getGoods_name()+"已下架", "");
            }
            //计算订单的费用
            //商品总价
            ...
            String specification = productInfo.getGoods_specification_ids();
            cartVo.setList_pic_url(productInfo.getList_pic_url());
            if(null!=specification && !"".equals(specification)){
                ...
                cartVo.setList_pic_url(picture);
            }
