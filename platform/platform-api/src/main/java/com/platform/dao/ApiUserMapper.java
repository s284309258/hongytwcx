...
public interface ApiUserMapper extends BaseDao<UserVo> {
    ...
    int updateLevelUnderUserDaili(@Param("user_id") Long user_id);

    int updateLevelUnderUserHuoheren(@Param("user_id") Long user_id);

    int insertBankAccount(@Param("userVo") UserVo userVo);

    int updateTaskNum0(@Param("user_id") Long user_id);
}
